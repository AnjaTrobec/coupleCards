...
CATEGORY_COLORS = {
    ...
    "PRILJUBLJENE": (1, 0.85, 0.4, 1), # Zlata barva za priljubljene
}
...
class CardApp(App):
    ...
    def show_menu(self):
        ...
        # Glavni vsebnik menija
        menu_content = BoxLayout(orientation="vertical", padding=[30, 10, 30, 30], spacing=20)

        # 1. HEADER (Srček + Naslov)
        header = BoxLayout(orientation="vertical", size_hint=(1, 0.35))
        heart = Label(text="❤️", font_size=100, size_hint=(1, 0.6))
        title = Label(
            text="NAJUH POGOVOR", 
            font_size=42, 
            font_name=FONT_PATH,
        ...
        subtitle = Label(
            ...
            font_size=18, 
            font_name=FONT_PATH,
            color=(0.6, 0.2, 0.4, 0.6),
            size_hint=(1, 0.2)
        )
        header.add_widget(heart)
        header.add_widget(title)
        header.add_widget(subtitle)
        menu_content.add_widget(header)

        # 2. SEZNAM KATEGORIJ (Ena kolona)
        grid = GridLayout(cols=1, spacing=15, size_hint=(1, 0.65))
        
        # Priprava vrstnega reda (Priljubljene gredo na konec)
        ...
        if "PRILJUBLJENE" in self.questions:
            grid.add_widget(Widget(size_hint_y=None, height=10)) # Razmik
    ...
    def add_menu_button(self, container, category, color):
        """Ustvari lep zaobljen gumb za meni"""
        btn = Button(
            text=category.upper(), 
            font_size=26, 
            font_name=FONT_PATH,
            color=TEXT_COLOR,
            background_normal="", 
            background_color=(0,0,0,0), # Prozoren gumb, ker rišemo RoundedRect
            size_hint_y=None,
            height=75
        )
    ...
    def choose_card_count(self, category):
        ...
        for n in [5, 10, "Vse"]:
            ...
            btn = Button(
                text=str(n), size_hint=(1, None), height=60, 
                font_name=FONT_PATH, font_size=24,
    ...
    def show_question(self):
        ...
        info_label = Label(
            ...
            color=TEXT_COLOR, font_size=22, font_name=FONT_PATH, size_hint=(1, 0.1)
        )
        ...
        q_label = Label(
            text=current_q, font_size=34, font_name=FONT_PATH,
            halign="center", valign="middle", color=TEXT_COLOR
        )
        q_label.bind(size=q_label.setter("text_size"))
        card.add_widget(q_label)
        main_content.add_widget(card)

        # 3. Vrstica z gumbi (4 gumbi)
        btn_grid = GridLayout(cols=4, spacing=10, size_hint=(1, 0.25))
        
        btn_prev = Button(text="NAZAJ", background_color=BTN_COLOR, background_normal="", font_name=FONT_PATH, font_size=18)
        btn_prev.bind(on_press=self.prev_question)
        
        btn_menu = Button(text="MENI", background_color=BTN_COLOR, background_normal="", font_name=FONT_PATH, font_size=18)
        btn_menu.bind(on_press=lambda x: self.show_menu())
        
        # Zvezdica v RelativeLayoutu
        is_fav = current_q in self.favorites
        fav_container = RelativeLayout()
        self.fav_btn_click = Button(background_color=BTN_COLOR, background_normal="", text="")
        ...
        btn_next = Button(text="NAPREJ", background_color=BTN_COLOR, background_normal="", font_name=FONT_PATH, font_size=18)
        btn_next.bind(on_press=self.next_question)

        btn_grid.add_widget(btn_prev)
        btn_grid.add_widget(btn_menu)
        btn_grid.add_widget(fav_container)
        btn_grid.add_widget(btn_next)
        
        main_content.add_widget(btn_grid)
    ...
    def show_game_over(self):
        self.root_layout.clear_widgets()
        l = Label(
            text=f"Prišla sta do konca kategorije!", 
            font_size=36, font_name=FONT_PATH,
            color=TEXT_COLOR, halign="center"
        )
        b = Button(
            text="Nazaj Domov", size_hint=(None,None), size=(240,70), 
            font_name=FONT_PATH, font_size=24,
            pos_hint={'center_x':.5}, background_color=BTN_COLOR, background_normal=""
        )
        b.bind(on_press=lambda x: self.show_menu())
