...
# 1. KONFIGURACIJA STRANI (Layout 'centered' je bolj stabilen za centriranje)
st.set_page_config(page_title="Najina Pot", page_icon="❤️", layout="centered")
...
# 3. ZMAGOVALNI CSS STIL - KONČNA VERZIJA ZA IPHONE
st.markdown(f"""
    <style>
    ...
    /* --- VARNA DEFINICIJA IKONE NA VRHU (Mala, v belem polju) --- */
    /* Sliki ukažemo, naj bo ožja in naj "poleti" navzgor na beli header */
    [data-testid="stImage"] img {{
        max-width: 80px !important; /* Še manjša ikona, da je varna */
        margin-left: auto !important;
        margin-right: auto !important;
        /* Negativni rob, ki sliko prestavi navzgor na beli header */
        margin-top: -110px !important; 
        position: relative !important;
        z-index: 9999 !important; /* Srček naj bo čez vse */
    ...
    /* KLJUČ ZA CENTRIRANJE KATEGORIJ NA IPHONU */
    /* To prisili Safari, da gumbe in naslove v kremnem delu centrirana */
    .force-center [data-testid="stVerticalBlock"] > div {{
    ...
    /* ANIMACIJA LETEČIH SRČKOV (Custom HTML, Safari strojno pospeševanje) */
    ...
    .heart-particle {{
        ...
        pointer-events: none;
        z-index: 9999;
        /* Tole spodaj Safariju ukaže strojno pospeševanje na iPhonu */
...
def show_top_icon():
    ...
    if os.path.exists("image_8.png"):
        # Širina je nastavljena na 150px, CSS jo zmanjša na 80px, da je varna
        st.image("image_8.png", width=150, use_column_width=False)
    else:
        st.error("Manjka slika image_8.png v mapi!")
...
# STRAN 1: GLAVNI MENI Z IKONO NA VRHU
if st.session_state.page == "main":
    # BELI HEADER - ČEZ CELO ŠIRINO
    st.markdown(f'<div class="header-section">', unsafe_allow_html=True)
    st.markdown(f'<h1 style="font-size: 42px; margin: 0; visibility: hidden;">ČAS ZA NAJU</h1>', unsafe_allow_html=True) # Skrijemo stari naslov v headerju
    st.markdown('</div>', unsafe_allow_html=True)
    
    # KREMNI DEL - FOKUS NA CENTRIRANJE IN PREMEŠČEN SRČEK
    # Vse zapremo v force-center, da Safari centrirani
    st.markdown('<div class="force-center">', unsafe_allow_html=True)
    
    # 1. Prikaževa ikono srčka (CSS jo prestavi na beli header)
    show_top_icon()
    
    # 2. Prikaževa naslov in podnaslov v kremnem delu
    st.markdown(f'<h1 style="font-size: 42px; margin-top: -30px;">ČAS ZA NAJU</h1><p style="font-size: 18px; margin-top: 5px; opacity: 0.8;">✨ Pogovor, ki povezuje ✨</p>', unsafe_allow_html=True)
    ...
    if st.session_state.index < len(st.session_state.deck):
        ...
        with col2:
            if st.button("Meni"):
                ...
                st.rerun()
        with col3:
            # Uporabimo srčke namesto zvezdic
            is_f = current_q in st.session_state.favorites
            heart_emoji = "❤️" if is_f else "🤍" 
            if st.button(heart_emoji):
