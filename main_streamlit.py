...
if st.session_state.page == "main":
    st.markdown(f"""
        <div class="header-section">
            <h1 style='font-size: 42px; margin: 0;'>ČAS ZA POGOVOR</h1>
            <p style='font-size: 20px; margin-top: 5px; opacity: 0.8;'>✨ Za povezanost na globlji ravni ✨</p>
        </div>
        """, unsafe_allow_html=True)
    
    st.markdown('<div class="menu-container">', unsafe_allow_html=True)
    st.markdown(f"<h2 style='text-align: center; color: {TEXT_COLOR}; font-size: 32px; margin-bottom: 20px;'>IZBERI KATEGORIJO:</h2>", unsafe_allow_html=True)
    
    categories = sorted([c for c in questions.keys()])
    for cat in categories:
        # TRIK: Trije stolpci za centriranje gumba
    ...
    st.markdown('<div class="menu-container">', unsafe_allow_html=True)
    st.markdown('<p style="text-align: center; font-size: 22px;">Koliko kartic želita vleči?</p>', unsafe_allow_html=True)
    for opt in [5, 10, "Vse"]:
        c1, c2, c3 = st.columns([0.1, 0.8, 0.1])
        with c2:
    ...
    if st.session_state.index < len(st.session_state.deck):
        ...
        st.markdown(f"<p style='text-align: center;'>Kartica {st.session_state.index + 1} od {len(st.session_state.deck)}</p>", unsafe_allow_html=True)
        st.markdown(f'<div class="q-card"><p class="q-text">{current_q}</p></div>', unsafe_allow_html=True)
        
        col1, col2, col3, col4 = st.columns([1, 1, 0.8, 1.2])
        with col1:
            if st.button("<"):
        ...
        st.balloons()
        st.markdown("<div class='menu-container' style='text-align:center;'><h2>Konec! ❤️</h2></div>", unsafe_allow_html=True)
        c1, c2, c3 = st.columns([0.1, 0.8, 0.1])
        with c2:
            if st.button("Nazaj"):
                st.session_state.page = "main"
